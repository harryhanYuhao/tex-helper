...
//! 1. The state of formatting shall be recorded. We can not have a purely recursively descent
//!    formatter as the parser, as sometimes the formatted lines may change in order
//!
//! The is how we tackle the problem
//! Use FormatState struct to record the state of formatting
//! Use FormatRes struct to record the result of formatting
//!
//! For each node type, create a recursive formatting function just like the recursive descent
//! parser.
//! Mutable FormatState and FormatRes are passed to each function
//!
...
struct Package {
    name: String,
    // Although the options are usually in square brackets, there may be several square brackets or curly brackets
    // so the options are simple saved as a string containing all the options and brackets.
    options: String, 
}
...
/// This struct is necessary, as formattor will
#[derive(Debug)]
struct FormatRes {
...
// Recall the definition of NodeType below:
//enum NodeType {
//    Passage, // A passage consists of many paragraphs
...
//
pub fn format(ast: NodePtr, config: &Config) -> Result<String, Box<dyn Error>> {
